...
rattle_path = join(evaluation_path,'rattle','rattle-cli.py')
dataset_path = join(evaluation_path,'Dataset','source/')
tmp_file_path = join(evaluation_path,'tmp','tmp_file.bytecode')
result_path = join(evaluation_path,'results/')
csv_path = join(evaluation_path,'tx_top1w_rattle.csv')
...
if __name__ == '__main__':
   ...
   # os.chdir(target_path)
    ...
    for file in os.listdir(dataset_path):
        ...
        try:
                return_info = subprocess.run(['python3.8',rattle_path,'--cfg',result_path+filename+'.dot','-i',tmp_file_path,'-nsf'],timeout=120,check=True,capture_output=True)
