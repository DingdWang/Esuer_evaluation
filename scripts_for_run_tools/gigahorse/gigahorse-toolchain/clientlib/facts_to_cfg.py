...
def construct_cfg() -> Tuple[Mapping[str, Block], Mapping[str, Function]]:
    ...
    function_call = load_csv_map('IRFunctionCall.csv')
    func_calls = {}
    for caller,callee in function_call.items():
        caller_func = tac_block_function[caller]
        if caller_func not in func_calls:
            func_calls[caller_func] = {}
        if callee not in func_calls[caller_func]:
            func_calls[caller_func][callee] = 0
        func_calls[caller_func][callee] += 1
    
    with open("function_calls.json",'w') as f:
        json.dump(func_calls,f,indent=4)
    
    # Store merge and branch
    preds_and_succs = {"merge":{},"branch":{}}
    for bb, preds in tac_block_pred.items():
        if len(preds) > 1:
            func = tac_block_function[bb]
            if func not in preds_and_succs["merge"]:
                preds_and_succs['merge'][func] = {}
            if len(preds) not in preds_and_succs['merge'][func]:
                preds_and_succs['merge'][func][len(preds)] = 0
            preds_and_succs['merge'][func][len(preds)] += 1
        
    for bb, succs in tac_block_succ.items():
        if len(succs) > 1:
            func = tac_block_function[bb]
            if func not in preds_and_succs["branch"]:
                preds_and_succs['branch'][func] = {}
            if len(succs) not in preds_and_succs['branch'][func]:
                preds_and_succs['branch'][func][len(succs)] = 0
            preds_and_succs['branch'][func][len(succs)] += 1

    with open("merge_and_branch.json",'w') as f:
        json.dump(preds_and_succs,f,indent=4)
